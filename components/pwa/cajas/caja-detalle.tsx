...
import { SheetRegistrarMovimiento } from "@/components/pwa/cajas/sheet-registrar-movimiento";
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative px-6 pt-2 pb-4">
        <div
          style={{
            viewTransitionName: `caja-banner-${caja.empresaId}`,
            background: `linear-gradient(135deg, ${color}, ${oscurecerColor(color, 0.3)}, ${oscurecerColor(color, 0.65)})`,
          }}
      ...
      <section className="flex flex-col gap-6 px-8 pb-8">
        ...
        {movimientosDelDia.length > 0 && (
          <div className="flex flex-col gap-4 rounded-2xl border border-gray-50 bg-white p-4 shadow-sm">
            {movimientosDelDia.map((movimiento) => {
              ...
              return (
                <button key={movimiento.id} type="button" onClick={() => setMovimientoSeleccionado(movimiento)} className="flex w-full items-center justify-between gap-3 text-left active:opacity-70">
                  <div className="flex min-w-0 items-center gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl" style={{ backgroundColor: colorConAlpha(colorCategoria, 0.12), color: colorCategoria }}>
                      <Icono className="h-5 w-5" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="truncate font-semibold text-gray-800">{nombre}</h4>
                      <p className="text-xs text-gray-400">{formatearHora(movimiento.fecha)}</p>
                    </div>
                  </div>
                  <p className={`shrink-0 font-mono text-sm font-bold whitespace-nowrap ${movimiento.tipo === "ingreso" ? "text-emerald-600" : "text-red-600"}`}>
